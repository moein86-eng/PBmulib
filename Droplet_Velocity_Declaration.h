#ifndef __DROPLET_VELOCITY_DECLARATION__
#define __DROPLET_VELOCITY_DECLARATION__

#include<string>

//#include "pch.h"

class Droplet_Velocity
{
public:
	Droplet_Velocity(double* Grid_Points_, unsigned int Grid_Number, std::string Formula_str, std::string Variable_str, std::string VarType_str, const std::string* Parameter_str, double* Parameter_val, unsigned int Parameter_number);   //constructor used for user-specified relation using parser
	Droplet_Velocity(double* Grid_Points_, unsigned int Grid_Number, std::string ModelName_str, double* Parameter_val);   //constructor used for included models from literature
	~Droplet_Velocity();
...
public:
	double** Vd;						  //array for interfacial Coalescence rate. 
};
